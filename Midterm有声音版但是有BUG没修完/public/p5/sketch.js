...
let rects = []; // 定义一个数组来存储所有的矩形
let fillColor; // 生成具有随机透明度的颜色
...
rainSound = loadSound('raining.wav'); // 确保音频文件路径正确
...
function preload() {
  ...
  rainSound = loadSound('raining.wav'); // 加载音频文件
  console.log("preload done");

}

function keyPressed() {
  // Start the sound only if it hasn't been started yet
  // This checks if the 'W' key was pressed
  if (key === 'W' || key === 'w') {
...
function draw() {
   background(0); // 清除画布
    ...
    if (wPressCount > 30 && rainSound.isPlaying()) {
     rainSound.stop(); // 停止播放背景音乐
    }
